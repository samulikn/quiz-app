...
const Heading = ({ subject = "" }): ReactNode => {
  ...
  return (
    <header className="w-full px-6 py-4 flex items-center sm:px-8 xl:px-5 xl:py-15">
      <div className="flex items-center gap-4">
        <svg className="w-10 h-10"></svg>
        <h2>{subject}</h2>
      </div>
      <div className="ml-auto flex items-center gap-2 sm:gap-4">
        <img src={theme === "light" ? IconSunDark : IconSunLight} alt="icon sun"
        ...
        <img src={theme === "light" ? IconMoonDark : IconMoonLight} alt="icon moon" className="w-4 h-4 sm:w-6 sm:h-6" />
      </div>
    </header>
  );
};
